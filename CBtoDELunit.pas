...
//	symArray   :TArray<TSymbol>;
	come       :STRING;
	str        :STRING;
	varstr :STRING;      //ïœêîêÈåæÇÇΩÇﬂÇÈ
	conststr: string;    //íËêîêÈåæÇÇΩÇﬂÇÈ
	êÈåæÉuÉçÉbÉN:boolean;
...
begin
	...
	if len=0 then exit; //Ç∑Ç◊ÇƒçÌèúï∂éö
	si:=1;
	...
	var
		...
		begin
			...
			if syms[i].typ=symCBlock then begin
				...
				if not ÉCÉt then result:=result+';'+CRLF;
		...
		function incget(i:integer):string;   //àÍÇ¬ì«ÇÒÇ≈êiÇﬁ
		begin
			result:=get(i);
			inc(pos);
		end;

		function incgetval(i:integer):string;   //àÍÇ¬ì«ÇÒÇ≈êiÇﬁÉuÉçÉbÉN
		begin
		...
		function incgetsrc(i:integer):string;   //àÍÇ¬ì«ÇÒÇ≈êiÇﬁÉuÉçÉbÉN
		begin
			result:=syms[posstart+i].str;
			inc(pos);
		end;
		...
		function cat(pos:integer):string;   //ç≈å„Ç‹Ç≈Ç≠Ç¡Ç¬ÇØÇÈ ç≈å„ÇÃÅGÇ»Çµ
		var i:integer;
		begin
			result:='';
...
//				result:=dtrimstring(result,' ;');
		...
		function inccat:string;   //ç≈å„Ç‹Ç≈Ç≠Ç¡Ç¬ÇØÇÈ
		begin
		...
		begin
			...
			result:=syms.structureisstr(posstart,SplitString(a,'Åb'));
		end;
...
//						end;
	...
	begin
		...
		pos:=0;
		bindent:=indent;
		//ÉuÉçÉbÉNÇ…ïœä∑ëOÇï€ë∂ÇµÇƒçƒç\ê¨Ç≈Ç´ÇÈÇÊÇ§Ç…Ç∑ÇÈ
		try
...
//				dmessage(strc);
	...
	function éÆ(ç≈èâ:boolean): boolean;
	var
		ââéZéq:array of TStringDynArray;
		count:integer;
		var Ç†Ç¡ÇΩ:boolean;

		function ÉJÉbÉRÇ¬ÇØÇÈ(ipos: integer): boolean;
		var cpos1,cpos2: integer;
		begin
			if ipos>High(ââéZéq) then exit(false);
			cpos1:=syms.hi;
			result:=ÉJÉbÉRÇ¬ÇØÇÈ(ipos+1);
			if MatchText(sym.str, ââéZéq[ipos]) then begin
				repeat
					syms.add(sym);
					symnext;
					syms.add(sym);
					symnext;
					result:=ÉJÉbÉRÇ¬ÇØÇÈ(ipos+1);
				until not MatchText(sym.str, ââéZéq[ipos]);
				cpos2:=syms.count+1;
...
//					if syms.count>0 then //ëOÇÃóvëfÇ…ë´Ç∑
				dsrc:=dsrc+sym.str;
			...
			if str='if' then begin
				...
				ckakko;
				continue;
			end else if éÆ(true) then begin
				if str=breakchar then break;
			end;
			...
			if not â¸çsÇ‹Ç≈ then begin
                if str=':' then begin

					if psym.typ=symKBlock then continue;    //ëOÉJÉbÉRÇÕÉÅÉìÉoèâä˙âªèâä˙âª
					if êÈåæÉuÉçÉbÉN then continue;//label:
					break;
				end;
			end;
...
begin
	sêÈåæÉuÉçÉbÉN:=êÈåæÉuÉçÉbÉN;
	syms.null.clear;
	statement;
	result:=reconstruct;
	êÈåæÉuÉçÉbÉN:=sêÈåæÉuÉçÉbÉN;
end;
...
begin
	if t<0 then t:=0;
	if t>count then t:=count; //îÕàÕí¥ÇÕí«â¡
...
//		end;
	...
	begin
		result:=true;
			...
			if arg[si]='ç∂' then begin
				if sym.typestr<>'ï∂' then
					exit(false);
				exit(true);
			end else
			if arg[si]='âE' then begin
				if typestr<>'êî' then
					if typestr<>'ï∂' then
						if typestr<>'ï∂éöóÒ' then
...
begin
	...
	if typ=symNumber       then exit('êî');
	if typ=symChar         then exit('ï∂éö');
	if typ=symInt          then exit('Int');
	if typ=symDouble       then exit('Double');
	if typ=symString       then exit('ï∂éöóÒ');
	...
	if typ=symOperator     then exit('ââéZéq');
	if typ=symCommand      then exit('ñΩóﬂ' );
	if typ=symLabel        then exit('ÉâÉxÉã'   );
	if typ=symDelimiter    then exit('ãÊêÿ');
	if typ=symEnd          then exit('End'      );
	if typ=symJump         then exit('Jump'     );
	if typ=symQualifier    then exit('èCè¸éq');
	if typ=symReserved     then exit('ï∂' );
	if typ=symFunction     then exit('ä÷êî' );
	if typ=symVariable     then exit('ïœêî' );
	if typ=symArray        then exit('îzóÒ'    );
end;
...
begin
	...
	for i:=0 to yw do begin
		...
		for j:=0 to xw do begin
			if j>0 then s:=s+#9;
				ss:=StringReplace(Cells[xs+j,ys+i],#9,'',[rfReplaceAll]);//É^ÉuÇÕçÌèú
...
finalization
	Ini.Free;
				...
				//ñΩóﬂ
//				if sis('*.ContainsÅb(*)') then begin   //èWçá
//					s:=' ('+incgetsrc(1)+' in '+StringReplace(incgetsrc(0),'.Contains','',[])+') ';
//
//				end else

//				if sis('ifÅb(*)') then begin   //if
//					s:=incgetsrc(0)+' '+incgetsrc(1)+' then ';
//				end else
//				if sis('doÅb{*}ÅbwhileÅb(*)') then begin   //do while falseÇ≈èIóπ   repeat until trueÇ≈èIóπ
//					incgetsrc(0);
//					incgetsrc(2);
//					s:='repeat'+CRLF+incgetsrc(1)+' until not ( '+incgetsrc(3)+' ) ';
//				end else
//				if sis('whileÅb(*)') then begin   //while
//					s:=incgetsrc(0)+' '+incgetsrc(1)+' do ';
//				end else
//				if sis('return') then begin   //return éÆ;  exit(éÆ)Å@
//					incget(0);
//					s:='Exit( '+inccat+' )'
//				end else

//				if sis('forÅb(*)') then begin   //if
//					incgetsrc(0);
//					s:=incgetsrc(1);
//					param:=SplitString(s,';');
//					s:=makefor;
...
//				end else
//
//				if sis('classÅbï∂Åb{*}') then begin
//					s:=incgetsrc(1)+' = '+incgetsrc(0)+CRLF+incgetsrc(2)+CRLF;
//					s:=s+indenttab+'end;'+CRLF;
//
//				end else
//				if sis('classÅbï∂Åbï∂Åb{*}') then begin
...
//				if sis('#defineÅbï∂Åb(*)') then begin//#define ñºëO() ÇÕä÷êîÇ÷Å@àÍóÒÇ∑Ç◊Çƒì«ÇﬁèàóùÇÇ¢ÇÍÇÈ
//					incgetsrc(0);
...
//
//				end else
//				if sis('#defineÅbï∂') then begin//#define ñºëO íl
//					incgetsrc(0);
...
//				end else
				...
				//#define a 1*2  åvéZ
				//#define acb(d) 123*d  ()Ç†ÇËÇÕéÆÇÕä÷êîÅH

				//6
//				if sis('ï∂Åb&Åbï∂Åb::Åbï∂Åb(*)') then begin //ä÷êî éQè∆
...
//
//				end else
//				if sis('ï∂Åb*Åbï∂Åb::Åbï∂Åb(*)') then begin //ä÷êî É|ÉCÉìÉ^
//					s:='function '+incget(2)+'.'+incget(4)+' '+incget(5)+' : '+incget(0);
//					come:=incget(1);
//					stype:=symFunction;
//					incget(6);
...
//				end else
//				//4
//				if sis('ï∂Åb&Åbï∂Åb(*)') then begin //ä÷êî éQè∆
//					s:='function '+incget(2)+' '+incget(3)+' : '+incget(0);
//					come:=incget(1);
//					stype:=symFunction;
//
//				end else
//				if sis('ï∂Åb*Åbï∂Åb(*)') then begin //ä÷êî É|ÉCÉìÉ^
//					s:='function '+incget(2)+' '+incget(3)+' : '+incget(0);
//					come:=incget(1);
//					stype:=symFunction;
//
//				end else
//				if sis('èCè¸éqÅbï∂Åb&Åbï∂') then begin //êÈåæ const éQè∆Å@%0 %3 : %1
//					s:=incget(0)+' '+incgetval(3)+' : '+incget(1);
//					come:=incget(2);
//				end else
//				if sis('èCè¸éqÅbï∂Åb*Åbï∂') then begin //êÈåæ const É|ÉCÉìÉ^
//					s:=incget(0)+' '+incgetval(3)+' : '+incget(1);
//					come:=incget(2);
//
//				end else
//				if sis('èCè¸éqÅbï∂Åbï∂') then begin     //êÈåæ const
//					s:=incget(0)+' '+incgetval(2)+' : '+incget(1);
//
//				end else
				//3
//				if sis('ï∂Åb&Åbï∂') then begin //êÈåæ éQè∆
//					s:=incgetval(2)+' : '+incget(0);
//					come:=incget(1);
//
//				end else
//				if sis('ï∂Åb*Åbï∂') then begin //êÈåæ É|ÉCÉìÉ^
//					s:=incgetval(2)+' : '+incget(0);
//					come:=incget(1);
//
//				end else
//				//2
//				if sis('ï∂Åb:') then begin  //ÉXÉRÅ[Év
//					s:=incgetsrc(0);
//					incget(1);
//					stype:=symLabel;
//
//				end else
//				if sis('ç∂Åbï∂') then begin  //êÈåæ
//					s:=incgetval(1)+' : '+incget(0);
//
//				end else
//				if sis('(*)Åbï∂') then begin //ÉLÉÉÉXÉg
//					s:=Split(incgetsrc(0),0)+'( '+incget(1)+' )'
//
//				end else
//				if sis('(*)Åb(*)') then begin //ÉLÉÉÉXÉg
//					s:=incget(2)+' : '+incget(0);
//				end else
...
//					if syms[pos].str='=' then //êÈåæÇÃéüÇ™ë„ì¸
...
//					if vname='' then exit;            //êßå‰ïœêîÇ»Çµ
//					if vname<>param2[0] then exit;    //êßå‰àŸÇ»ÇÈ
//					if vname<>param3[0] then exit;    //êßå‰àŸÇ»ÇÈ
//
...
//	function ââéZéqèáèò(ç≈èâ:boolean): boolean;
//	var
//		ââéZéq:array of TStringDynArray;
//		óDêÊ:TStringDynArray;
//		count:integer;
//
//		function ÉJÉbÉRÇ¬ÇØÇÈ(ipos: integer): boolean;
//		var cpos1: integer;
...
//			if ipos>High(ââéZéq) then exit(false);
//			cpos1:=syms.hi;
//			Ç†Ç¡ÇΩ:=ÉJÉbÉRÇ¬ÇØÇÈ(ipos+1);
//			if MatchText(sym.str, ââéZéq[ipos]) then begin
//				óDêÊÇæ:=MatchText(sym.str, óDêÊ);
//				repeat
//
//					syms.add(sym);
//					symnext;
...
//				until not MatchText(sym.str, ââéZéq[ipos]);
//				inc(count);
//				if óDêÊÇæ and Ç†Ç¡ÇΩ then begin
////					if Ç†Ç¡ÇΩ then cpos2:=syms.count+1;
